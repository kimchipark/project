...
#공백,개행 등을 없앤다.
def ObjGetText(soup,selecter,default,attr=""):
    #========================================
    value = ""
    if attr == "" :
...
def GetGender(html,start_pos) :
    ...
    gender = ""
    e_pos  = -1
    s_pos  = html.find('"iPer": ',start_pos)
    if s_pos == None :
        gender = "없음"
    else :
        if s_pos > 0  :
            e_pos = html.find(',',s_pos)
            if e_pos > 0  :
                s_pos += 8   
                gender = html[s_pos:e_pos]
        #실수 -> 반올림 작업
        if gender == "" :
            gender = "없음"
        else :
            gender = float(gender[:4])
            gender = round(gender) 
    return gender, e_pos
    #========================================
...
def GetMoveInfo(code,filename) :
    ...
    html  = requests.get(mURL,headers=agent).text
    soup  = BeautifulSoup(html, "html.parser")    

        
    #관람객 평점 추출 
    score = ObjGetText(soup, "#pointNetizenPersentBasic", "0")
    score = score.replace("관람객 평점","")
    #print("관람객 평점:",score)  
    
    #줄거리
    text = ObjGetStory(soup,"p.con_tx","내용없음")
    #print(text)
    ...
    if len(span_item) >= 4 :       
        ...
        #러닝타임
        time = ObjGetText(soup,"p > span:nth-child(3)","알수없음")
        #print("러닝타임:",time)
